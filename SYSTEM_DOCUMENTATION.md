...
void execute();
void cancel();
void abortWithError(QCurl::NetworkError error, const QString &message = QString());
void pauseTransport(QCurl::PauseMode mode = QCurl::PauseMode::All);
void resumeTransport();

// 读取数据
std::optional<QByteArray> readAll();
std::optional<QByteArray> readBody();
...
QList<RawHeaderPair> rawHeaders() const;
QByteArray rawHeader(const QByteArray &headerName) const;
...
`QCNetworkCache` 的读取入口是 `lookup(url, ReadMode)`。旧 `data()`、`contains()`、`metadata()` 读取 API 已移除；需要未过期条目时使用 `lookup(url, FreshOnly)`，需要忽略过期时间时使用 `lookup(url, AllowStale)`。命中判断使用 `LookupStatus` 或 `hit()`，不能用 `body.isEmpty()` 判断 miss，因为零字节响应会返回 hit 状态和空 body。
...
// 单文件 multipart 流式上传
QFile sourceFile("/tmp/upload.bin");
sourceFile.open(QIODevice::ReadOnly);

QCNetworkReply *reply = manager->postMultipartDevice(
...
PUT 与 `postMultipartDevice()` 必须有已知长度。Async raw-body 可通过 `readyRead()` 从
source-not-ready 恢复，Sync raw-body 遇到 `read() == 0 && !atEnd()` 会失败。
...
manager->addMiddleware(&custom); // manager 不持有 middleware 所有权
```
...
public:
    QCNetworkCacheLookupResult lookup(const QUrl &url,
                                      QCNetworkCacheReadMode mode) override {
        const QString key = url.toString();
        const auto entry = redis->read(key);
        if (!entry.has_value()) {
            return {};
        }
    ...
    void insert(const QUrl &url, const QByteArray &data,
        ...
        redis->write(url.toString(), data, metadata);
    }

    bool remove(const QUrl &url) override {
        return redis->del(url.toString());
    }
    ...
    qint64 cacheSize() const override {
        return redis->bytesUsed();
    }

    qint64 maxCacheSize() const override {
        return maxBytes;
    }
