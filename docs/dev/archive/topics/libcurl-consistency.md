...
> 历史归档：以下原文仅反映记录时点，不定义当前 2.0 合同或发布资格；其中“当前”按原始上下文阅读。 原状态板没有统一的 commit 绑定，完成度与行数不作为当前证据，收录基准也不追溯认证旧统计；不再更新此页。现行规则见[构建与测试](../../build-and-test.md#maintenance-rules)。
