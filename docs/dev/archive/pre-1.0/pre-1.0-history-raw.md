# Pre-1.0 raw history archive

> Internal raw archive copied from the previous `docs/internal/pre-1.0-history.md`. It is kept only for audit/search and does not define the current release contract.

# QCurl pre-1.0 internal history

本文保留 QCurl 公开 `1.0.0 first stable` 之前的维护者历史、旧日期流水和 pre-1.0 变更记录。它只用于内部追溯，不构成当前用户-facing release history。

## Pre-1.0 internal history (reference only)

以下记录仅用于维护者追溯，不构成 QCurl 1.0.0 当前用户-facing release history。
...
#### Breaking
- 该组 scheduler ABI 变更已纳入 `QCurl 1.0.0 / SOVERSION 1` first stable Core 合同；下游需要按当前 1.0.0 头文件和二进制重新构建
...
- 收紧 private header contract：`QCNetworkConnectionPoolManager` 移除对外 internal-only bridge，新增 `src/QCNetworkConnectionPoolManager_p.h` 承接 reply→connection-pool 内部协作；`CurlGlobalConstructor` / `QCNetworkLogRedaction` 保持在 `src/private/*_p.h`，不再进入 `QCURL_INSTALL_HEADERS`
  - 方案：202603241541_private-header-contract-audit（历史本地路径：`.helloagents/archive/2026-03/202603241541_private-header-contract-audit/`，未随产品版本控制）
...
- 一致性 gate（all）：`python3 tests/libcurl_consistency/run_gate.py --suite all --build`（45 passed，报告：`build/libcurl_consistency/reports/junit_all.xml`）
