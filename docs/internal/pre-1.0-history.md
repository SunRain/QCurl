...
本文是 QCurl pre-1.0、RC 与 hard-break 历史材料的内部索引。它只帮助维护者查找旧记录，不定义当前 `1.0.0 first stable` 发布合同。
...
- 查旧 release identity、RC 或 hard-break 背景：搜索 raw archive 中的版本号、日期或任务名。
...
| RC / 3.0 草稿 | 旧 RC release notes、hard-break migration guide、readiness 草稿 | 已移入 `archived-release/`，只作维护者参考 |
