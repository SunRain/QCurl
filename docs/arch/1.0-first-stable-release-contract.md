...
- 当前 baseline → 当前库的 clean ABI diff 证据：`build/abi/qcurl-core-v1.abidiff.txt`。
...
只通过 public header layout scan 不等于 Stable ABI ready。Fresh release 仍必须用当前库生成 baseline，并用同一当前库通过 clean ABI diff。历史 ABI 对比材料只保留在 `docs/internal/archived-release/`，不作为当前 release 阻断项或放行证据。
