...
后续仍不得把 `ExecutionMode::Sync`、libcurl callback typedef 或 live `QCNetworkAccessManager` QObject 状态重新混入 Core public contract。
...
这只说明 static 路径具备可验证入口，不等于 whole project static library ready。static `public-api` / `public-api-slow`、static consumer smoke、pkg-config 和 release gate 证据归档前，RC/Stable 报告必须继续保持 `static ready=false`。
