...
- `QCNetworkBody`、`QCNetworkMultipartBody`、`QCMultipartFormData` 和 transfer job 进入 Core helper/job surface；manager 只负责 `head()` / `get()` / `post()` / `put()` / `patch()` / `deleteResource()` / `sendCustomRequest()`。
...
`QCURL_BUILD_SHARED_LIBS=OFF` 已提供 static opt-in 构建形态，static export 可公开传播 Core 必需的 `CURL::libcurl`，并由 `QCurlConfig.cmake` 补齐 `find_dependency(CURL ...)`；`ZLIB::ZLIB` 不再进入默认 Core export。
...
Blocking Extras opt-in consumer smoke 现在还覆盖 `QCBlockingRequestOptions::setMaxInMemoryBodyBytes()`、bounded body、`downloadToDevice()`、`rawHeaders()`、`bytesReceived()` 与 `diagnosticCurlCode()`。Static opt-in gate 需在 `QCURL_BUILD_SHARED_LIBS=OFF` 构建目录中单独运行同一组 public-api / public-api-slow 检查。
