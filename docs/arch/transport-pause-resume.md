...
- `QCNetworkReply` 已支持可观测的 Paused 语义（`ReplyState::Paused` + `QCNetworkReply::isPaused()`），并提供 `pause(PauseMode)`/`resume()`（`src/QCNetworkReply.*`）。
...
- [x] **支持范围**：仅 `ExecutionMode::Async` 支持传输级 pause/resume；`Sync` 模式调用 `pause/resume` 必须 `qWarning()` 且 no-op。
...
- [x] **强制调用点**：任何成功的 `QCNetworkReply::resume()` 执行后必须触发 wakeup/kick（避免因缺少 socket/timer 事件导致长时间不推进）。
