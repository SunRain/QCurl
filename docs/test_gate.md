...
- `scripts/run_release_gate.py --tier fast|strict|full`
  - 2.0 release-candidate 发布门禁入口；不调用 `git`
  ...
  - `full`：在 strict 基础上增加完整 CTest、libcurl consistency full gate、动态符号 allowlist、capability matrix、sanitizer、Doxygen 和 release metadata scan；默认 `abiMode=none`
...
- `scripts/qcurl_abi_gate.py`
  - 为未来稳定 ABI 项目保留的显式诊断工具，不属于 2.0 默认发布门禁
  - `current` / `promotion-candidate` 被显式选择时，缺少 `abidw` / `abidiff`、共享库、头目录或调试信息仍 fail-closed
  - 动态符号 allowlist 检查导出面污染，不等于稳定 ABI 证明
- `public-api-slow`
  - 覆盖 Core-only component staging、export contract、Core consumer smoke
  ...
  - Core consumer fixture 必须覆盖 `QCNetworkRedirectConfig` / `QCNetworkTransferConfig`，证明 Request 配置族在 Core component 安装面可用
