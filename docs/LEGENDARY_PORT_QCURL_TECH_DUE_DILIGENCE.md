...
已执行的本地验证：在现有构建目录 `build/` 运行 `ctest --output-on-failure`，31/31 通过；运行 `python -m pytest -q tests/libcurl_consistency/test_env_smoke.py`，1/1 通过（`tests/libcurl_consistency/test_p0_consistency.py` 在当前环境下为跳过）。
...
| 协议：HTTP/3 | 非显式需求；对跨平台交付风险更高 | 网络库可选（建议可降级） | `src/QCNetworkHttpVersion.*`、`src/CurlFeatureProbe.*`；HTTP/3 相关 token 出现在 `src/QCNetworkHttpVersion.cpp` | `tests/tst_QCNetworkHttp3.cpp` 且 `ctest` 通过；成熟度中（依赖构建环境的 curl/QUIC 支持） | 部分满足（建议做运行时能力探测与降级策略固化） |
...
| TLS：证书钉扎（pinning） | 非必需，但对抗劫持/企业合规可能需要 | 网络库可选 | `src/QCNetworkSslConfig.h` 与 `src/QCNetworkReply_p.h` 存在 `CURLOPT_PINNEDPUBLICKEY` | 当前未看到对应命名测试；成熟度中 | 部分满足（建议增加可运行用例或至少文档化与错误行为测试） |
| 压缩：gzip/br 等 | Epic API 常用压缩；下载也可能 | 网络库可选但建议支持 | `src/QCNetworkRequest.h` 与 `src/QCNetworkReply.cpp` 使用 `CURLOPT_ACCEPT_ENCODING` | `tests/libcurl_consistency/test_p1_accept_encoding.py`（存在但当前环境门禁）；成熟度中 | 部分满足（建议将 accept-encoding 行为纳入可运行回归门禁） |
| 限速：发送/接收 | 大规模下载时的 QoS | 网络库可选 | `src/QCNetworkReply.cpp` 使用 `CURLOPT_MAX_RECV_SPEED_LARGE`/`CURLOPT_MAX_SEND_SPEED_LARGE` | 未见专门命名测试；成熟度中 | 部分满足 |
...
| 重试与退避：HTTP 5xx（500-504） | API 侧显式配置按 5xx 重试（`legendary-python/legendary/api/egs.py`：`Retry(status_forcelist=[500,501,502,503,504], allowed_methods={'GET'})`） | 上层与网络库均可；建议网络库提供通用机制，上层配置策略 | QCurl 在执行路径中基于 `CURLINFO_RESPONSE_CODE` 将 HTTP 4xx/5xx 映射为 `NetworkError` 并走统一重试逻辑（`src/QCCurlMultiManager.cpp` 与 `src/QCNetworkReply.cpp`）；策略层用 `QCNetworkRetryPolicy::shouldRetry(NetworkError, attemptCount)` 判定；默认 policy 已包含 500/502/503/504；并新增 opt-in 的 `QCNetworkRetryPolicy::retryHttpStatusErrorsForGetOnly` 用于对齐 legendary 的 `allowed_methods={'GET'}`；501 已有命名枚举 `NetworkError::HttpNotImplemented`，是否纳入默认可重试集合可按业务选择 | `tests/tst_QCNetworkRetryOffline.cpp` 覆盖 500/503 的离线重试语义；`tests/tst_QCNetworkRetry.cpp` 仍提供 httpbin 集成验证（环境缺失时会跳过） | 部分满足（5xx 重试能力与离线门禁已具备；若要严格对齐 `status_forcelist`，可考虑将 501 加入默认 retryableErrors 或由上层配置） |
| 重试与退避：HTTP 429 / Retry-After | Epic 侧限流时决定稳定性（429 + `Retry-After`） | 上层与网络库均可实现；建议网络库提供通用机制，上层配置策略 | 已补齐：`src/QCNetworkError.h`/`src/QCNetworkError.cpp` 增加 `HttpTooManyRequests(429)` 与命名描述；`src/QCNetworkRetryPolicy.*` 默认将 429 纳入 `retryableErrors`，并新增 `delayForAttempt(attemptCount, serverDelay)` 覆写入口（以 `maxDelay` cap）；`src/QCNetworkReply.cpp`（sync+mock）与 `src/QCCurlMultiManager.cpp`（async）解析 `Retry-After`（整数秒）并优先使用；另提供 opt-in 的 `retryHttpStatusErrorsForGetOnly` 以对齐 legendary 的 `allowed_methods={'GET'}` | 新增纯离线门禁 `tests/tst_QCNetworkRetryOffline.cpp` 覆盖 429+Retry-After 覆写与回退指数退避；`tests/tst_QCNetworkMockHandler.cpp` 覆盖 MockHandler 自动生效与序列/捕获能力 | 已满足（限流语义与离线回归门禁已具备；如需支持 HTTP-date 形式的 Retry-After 可作为后续增强） |
...
| 日志与可观测性：请求/响应/诊断 | 迁移调试与线上定位必须 | 网络库必须提供基础，上层决定采样与落盘 | `src/QCNetworkLogger.*`、`src/QCNetworkDiagnostics.*`、`src/QCNetworkDiagnostics.cpp` 含平台差异处理证据（`Q_OS_` 条件） | `tests/tst_QCNetworkLogger.cpp`、`tests/tst_QCNetworkDiagnostics.cpp` 且 `ctest` 通过；成熟度高 | 已满足 |
| 中间件/拦截器（注入 auth、统一重试策略、日志） | legendary 的 session/header 语义在移植时需要统一注入点 | 网络库建议提供钩子，上层配置 | 中间件接口与容器存在（`src/QCNetworkMiddleware.*`、`src/QCNetworkAccessManager.*`），但当前仍未见在请求执行路径中自动调用 `onRequestPreSend/onResponseReceived` 的证据（`src/**` 仅检出定义与 manager 存储）；MockHandler 已完成执行链路接入（`src/QCNetworkReply.cpp`），可用于纯离线回放与请求捕获 | `tests/tst_QCNetworkMiddleware.cpp` 仍以“容器与手动调用”为主；MockHandler 已补齐“自动生效”测试（`tests/tst_QCNetworkMockHandler.cpp`） | 部分满足（MockHandler 已可用；middleware 若要作为移植项目的稳定注入点仍需接入执行链路并补齐离线断言用例） |
...
| 跨平台差异：证书/代理/网络路径 | legendary 运行在 Win/macOS/Linux；网络库必须在发布包内“策略可控” | 网络库必须提供可配置项，上层负责落地策略 | 代码层面平台条件主要出现在 `src/QCNetworkDiagnostics.cpp`；发布集成相关在 `CMakeLists.txt`、`src/CMakeLists.txt`、`qcurl.pc.in`、`src/qcurl.pri` | 当前仅有本机 `ctest` 通过证据；对多平台 CI/证书打包策略不足以判断 | 部分满足（需要补齐跨平台 CI 与“CA/代理策略”验收，见第 7 节） |

## 2) 架构与接口评估

QCurl 的 `src/` 显示其架构定位非常明确：以 libcurl multi 为核心实现、以 Qt 的 QObject/信号槽作为异步抽象层，整体 API 形态接近 QtNetwork（存在 `QCNetworkAccessManager`/`QCNetworkRequest`/`QCNetworkReply` 三件套，分别位于 `src/QCNetworkAccessManager.*`、`src/QCNetworkRequest.h`、`src/QCNetworkReply.*`）。这对 Legendary 的 Qt/C++ 移植项目非常关键，因为上层可以用“请求对象 + reply 异步回调/信号”的方式替代 Python `requests.session()` 的习惯用法，并把 auth/header/cookie 等统一放到 manager 或上层 wrapper（例如 EgsClient）中实现（`src/QCNetworkRequestBuilder.*`、`src/QCRequestBuilder.*` 可直接注入 header）。需要注意的是：`src/QCNetworkMiddleware.*` 虽提供了中间件接口，但当前未见接入请求执行链路的证据，因此如果计划依赖 middleware 作为统一注入点，需要纳入补齐工作（见第 1.2 表与第 5.1 节）。
...
从规模与覆盖面看，QCurl 当前更像“可交付的基础网络库”而非“原型”：`src/` 下 C++ 头/源文件 74 个，总行数约 19.6k；`tests/` 下 Qt Test 用例文件 31 个，总行数约 15.5k；另有 Python 一致性/扩展测试目录 `tests/libcurl_consistency/`，其中 `test_*.py` 31 个，总行数约 7.0k。这个测试比重在网络库类项目中属于偏高水平，意味着很多边界条件已经被纳入自动化。
...
测试覆盖的缺口与最小可行补齐方案主要集中在两类：第一类是“存在测试文件但当前未被执行的门禁套件”。`tests/libcurl_consistency/` 下存在大量针对 cookie、redirect、accept-encoding、timeouts、pause/resume、httpauth、proxy、TLS verify 等的 Python 用例，但在当前环境中 `tests/libcurl_consistency/test_p0_consistency.py` 为跳过，说明其执行依赖特定服务/基线程序/环境变量。若要把 QCurl 作为移植项目的长期基础库，这套一致性测试应被纳入 CI 并可稳定执行；最小方案是明确并固化其 gate 条件（入口文件线索在 `tests/libcurl_consistency/run_gate.py`、`tests/libcurl_consistency/README.md`、`tests/libcurl_consistency/minimal_set.yaml`）。第二类是“Epic 迁移强关联但仍缺少可稳定复现的语义门禁”。这部分已通过“MockHandler 接入执行链路 + 纯离线 QtTest 门禁”补齐：`src/QCNetworkMockHandler.*` 与 `src/QCNetworkReply.cpp` 支持按 URL/method 的响应序列回放与请求捕获；`tests/tst_QCNetworkRetryOffline.cpp` 覆盖 500/503/429（含 `Retry-After` 覆写与回退指数退避）与 GET-only gating 的关键语义，从而避免 `tests/tst_QCNetworkRetry.cpp` 对本地 httpbin 的依赖成为“被跳过的门禁”。当前仍建议把 libcurl_consistency 的最小集跑通并纳入 CI，以提升跨平台交付可信度。

## 4) 完备性结论

结论等级：可直接使用（作为 `@legendary-python` Qt/C++ 移植项目的基础网络库）。

依据是可审计且可复现的：其一，QCurl 已覆盖移植必需的网络底座能力，并在 `src/` 中有明确模块分层与配置入口（请求对象 `src/QCNetworkRequest.h`、reply/执行与 I/O `src/QCNetworkReply.*`、事件循环与 multi 管理 `src/QCCurlMultiManager.*`、并发与连接池 `src/QCNetworkRequestScheduler.*`/`src/QCNetworkConnectionPoolManager.*`、TLS/代理/超时/重试等策略对象 `src/QCNetworkSslConfig.*`/`src/QCNetworkProxyConfig.*`/`src/QCNetworkTimeoutConfig.*`/`src/QCNetworkRetryPolicy.*`）；其二，自动化测试覆盖广且本机可跑通，`build/` 下 `ctest` 32/32 通过，包含 HTTP/2、HTTP/3、代理、WebSocket、缓存、连接池、取消、文件传输，以及新增的“离线重试/限流门禁”（`tests/tst_QCNetworkRetryOffline.cpp`）等关键路径的运行时证据；其三，legendary 的实际网络调用形态以 `requests.session()` + 超时 + 大量 HTTPS API + 下载为主（见 `legendary-python/legendary/api/egs.py`、`legendary-python/legendary/downloader/mp/*`），QCurl 的接口形态与能力集合可以自然承接。

需要强调的“阻塞项”更多在移植项目上层而非 QCurl 本体：OAuth/设备码登录流程、token 生命周期与持久化、manifest/chunk 业务规则、下载校验与存储布局等。这些建议保持在上层（第 6 节给出边界）。QCurl 侧更可能演变为阻塞的点集中在“限流语义（429/`Retry-After`）与可稳定回归的门禁”以及“跨平台交付一致性”：按 HTTP 5xx 状态码触发重试的能力与测试证据已存在（见 `src/QCCurlMultiManager.cpp`、`src/QCNetworkReply.cpp` 与 `tests/tst_QCNetworkRetry.cpp`），但 429/`Retry-After` 的自适应退避、以及与 legendary 对齐的 GET-only 重试语义仍需补齐并纳入离线门禁；同时不同平台证书/HTTP3 后端差异仍需 CI 固化（第 5/7 节给出可执行补齐路线）。
...
| 会话与 cookie jar（含持久化能力） | 部分满足 | 底层 cookie jar/注入具备，但 Web 登录的 XSRF cookie→header 流程与 Qt WebView 的 cookie 同步仍需上层明确 | `legendary-python/legendary/core.py`、`legendary-python/legendary/utils/webview_login.py`；`src/QCNetworkAccessManager.h`、`src/QCNetworkReply.cpp`、`src/QCCurlMultiManager.cpp`；`tests/libcurl_consistency/test_p1_cookiejar_1903.py`（门禁） |
| Web 登录 CSRF/XSRF（cookie→header） | 部分满足 | 可注入头与持久化 cookie，但缺少直接读取 cookie 值/与 Qt WebView cookie store 互通的明确方案 | `legendary-python/legendary/core.py`；`src/QCNetworkMiddleware.*`、`src/QCNetworkAccessManager.h`；建议补齐桥接与测试 |
...
| TLS 安全默认与可配置（CA/钉扎/禁用模式） | 部分满足 | default/insecure 已有；钉扎存在但缺少可运行回归门禁；跨平台 CA 策略需固化 | `src/QCNetworkSslConfig.*`、`src/QCNetworkReply_p.h`；`tests/tst_QCNetworkHttp2.cpp`、`tests/tst_QCNetworkHttp3.cpp` |
...
| CDN 选择与 https→http 降级（disable_https） | 部分满足 | 网络库支持 http/https 与多域名，但“允许 http/日志提示/安全默认”需上层与验收标准固化 | `legendary-python/legendary/core.py`、`legendary-python/legendary/cli.py`；`src/QCNetworkLogger.*`、`src/QCNetworkSslConfig.*` |
| 重试与退避（网络错误） | 已满足 | 无 | `src/QCNetworkRetryPolicy.*`；`tests/tst_QCNetworkRetry.cpp` |
| HTTP 5xx 状态码重试（500-504） | 部分满足 | 执行链路已按 `CURLINFO_RESPONSE_CODE` 将 5xx 映射为 `NetworkError` 并进入重试；默认策略覆盖 500/502/503/504，但 legendary 的 501 与 GET-only（`allowed_methods={'GET'}`）语义需要补齐/对齐 | `legendary-python/legendary/api/egs.py`；`src/QCCurlMultiManager.cpp`、`src/QCNetworkReply.cpp`、`src/QCNetworkRetryPolicy.h`；`tests/tst_QCNetworkRetry.cpp` |
| 限流语义（429/Retry-After） | 部分满足 | 可通过自定义 `retryableErrors` 触发 429 重试，但缺少 `Retry-After` 自适应退避与离线回归门禁 | `src/QCNetworkError.cpp`、`src/QCNetworkRetryPolicy.h`、`src/QCNetworkReply.cpp`；建议新增离线用例，见第 5 节 |
...
|---|---|---|---|---:|---|
| P0 | HTTP 429/`Retry-After` 自适应退避 + 与 legendary 对齐的 5xx/GET-only 语义 + 离线回归门禁 | 基于现有重试框架（`src/QCCurlMultiManager.cpp`、`src/QCNetworkReply.cpp`、`src/QCNetworkRetryPolicy.*`）补齐：解析响应头 `Retry-After` 并覆写重试延迟；补齐 429/501 的可重试默认集合或提供 status-forcelist 配置入口；确保与 legendary 的 `allowed_methods={'GET'}` 对齐（可在 QCurl 策略内实现，也可由上层对不同方法选用不同 policy）；新增离线 Qt Test（进程内 `QTcpServer`，或先把 `src/QCNetworkMockHandler.*` 接入执行链路后用于纯离线）覆盖 429+Retry-After/无 Retry-After、500/503 重试与上限行为 | 继续用 libcurl；不建议引入 QtNetwork 以免双栈 | 4–8 | 行为对齐的边界（方法白名单、最大等待上限、抖动策略）；门禁稳定性（避免依赖外部 httpbin/docker） |
...
| P1 | accept-encoding 与解压行为门禁 | 把 `tests/libcurl_consistency/test_p1_accept_encoding.py` 的门禁跑通，或补 C++ 侧可运行测试（mock 或本地 server） | 维持现有测试体系 | 2–5 | 测试环境依赖（本地服务/证书） |
| P1 | 证书钉扎（pinned public key）可运行回归 | 为 `CURLOPT_PINNEDPUBLICKEY` 增加可跑用例（本地 TLS server 或已知证书 fixture），并文档化失败错误码映射 | 不引入新 TLS 栈 | 3–7 | 本地 TLS server/证书 fixture 的维护成本 |
| P2 | HTTP/3 交付级降级策略与打包 | 运行时能力探测（已存在 `src/CurlFeatureProbe.*`）+ 明确降级顺序（H3→H2→H1.1）+ CI 在 Win/mac/Linux 上覆盖 | 继续 libcurl；不要把 HTTP/3 作为强依赖 | 5–15 | QUIC 后端差异、打包复杂度、回归成本 |
...
|---|---|---|---|---|
| status_forcelist 的表达 | 对齐 legendary 的 500/501/502/503/504 与后续扩展 | 保持现有 `QCNetworkRetryPolicy::retryableErrors`，并在示例/文档明确“HTTP 状态码可直接映射为 `NetworkError`”；已增强：在 `NetworkError` 增加 `HttpNotImplemented=501`、`HttpTooManyRequests=429`，且默认 retryableErrors 已包含 429（501 是否默认可重试仍建议按业务选择） | `src/QCNetworkError.*`、`src/QCNetworkRetryPolicy.*` | 不新增新类型即可通过 `static_cast<NetworkError>(code)` 配置任意 4xx/5xx，但可读性差；新增枚举值是向后兼容的增强 |
| Retry-After 支持 | 对齐 429 限流语义，避免“指数退避与服务端要求冲突” | 在延迟计算中引入“服务端建议等待时间”的覆写入口：例如新增 `delayForAttempt(int attemptCount, std::optional<std::chrono::milliseconds> retryAfter)`，若 `retryAfter` 存在则优先使用（并建议以 `maxDelay` 作为上限）；解析规则至少支持整数秒（可选支持 HTTP-date） | `src/QCNetworkRetryPolicy.*`（计算策略） + `src/QCCurlMultiManager.cpp`/`src/QCNetworkReply.cpp`（解析 headerData 并传入） | 建议把“解析”放在 reply 层（因为 reply 已持有原始 headerData）；策略层只负责“如何合并 retryAfter 与 backoff” |
...
首先是 OAuth/设备码登录与 token 刷新辅助。QCurl 不应该内置 Epic OAuth 流程，因为这会把第三方业务语义固化进基础库；推荐在移植项目上层实现“认证服务”，把 token 以可替换存储（内存/磁盘/系统钥匙串）管理，并通过上层 EgsClient/request builder 统一注入 `Authorization` 头（`src/QCNetworkRequestBuilder.*`、`src/QCRequestBuilder.*` 可直接设置 header）。如要把注入点下沉到 middleware，需要先补齐 `src/QCNetworkMiddleware.*` 与请求执行链路的集成（见第 1.2 表与第 5.1 节）。
...
|---|---|---|
| 认证登录 | 设备码或 webview 登录成功，token 可持久化并可刷新 | 上层新增认证模块与 EgsClient/request builder 注入 auth；如需统一注入点可补齐 middleware 集成（`src/QCNetworkMiddleware.*`） |
...
在不接触真实 Epic 服务的前提下，有两类问题无法仅凭当前运行结果完全确认：其一是“限流语义与幂等策略”是否满足长期稳定交付（建议重点审阅 `src/QCNetworkRetryPolicy.cpp`、`src/QCNetworkReply.cpp` 的重试触发条件，并补齐 5xx 与 429/Retry-After 回归）；其二是“libcurl_consistency 套件为何门禁跳过、如何在 CI 跑通”（建议按 `tests/libcurl_consistency/README.md`、`tests/libcurl_consistency/run_gate.py`、`tests/libcurl_consistency/minimal_set.yaml` 建立最小可跑配置并纳入 CI）。这些核查一旦完成，评估将从“本机行为可信”升级为“跨平台交付可信”。
