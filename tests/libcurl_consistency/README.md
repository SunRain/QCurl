...
运行方式（Gate/pytest 直跑）请参考：[`docs/dev/build-and-test.md`](../../docs/dev/build-and-test.md)。

### Python 依赖（锁定）

为保证复跑一致性，建议在 CI/验收环境按锁文件安装依赖（固定 pytest/websockets/brotli 等版本）：

- `python3 -m pip install -r tests/libcurl_consistency/requirements.lock.txt`

说明：
- `run_gate.py` 会在报告目录旁输出 `pip_freeze_<suite>.txt` 作为环境工件，用于审计解释与复跑对齐。
- 如已构建 `nghttpx-h3`，`run_gate.py` 也会 best-effort 输出 `nghttpx_version_<suite>.txt` 作为服务端能力快照工件。
...
- **客户端 API 输出（QCurl）**：`QCNetworkReply`/`QCWebSocket` 的信号序列、`error()`/`errorString()`、`readAll()`/`rawHeaderData()` 等（传输级 pause/resume 推荐 v2 `pauseTransport/resumeTransport`，旧 `pause/resume` 已弃用）。
...
- **不依赖本地 httpbin（`localhost:8935`）**：一致性 Gate 仅依赖上游 `curl/tests/http/testenv` 与本仓库 `http_observe_server.py`；本地 httpbin 仅用于 `tests/` 下的 QCurl 单侧集成测试（见 `tests/README.md`），服务不可用时应可跳过，不得作为一致性门禁前置条件。
...
- **baseline / QCurl**：同一用例在 libcurl baseline 与 QCurl 侧执行器（`tests/libcurl_consistency/tst_LibcurlConsistency.cpp`）下生成的 artifacts。
...
| HTTP/3 | 请求语义摘要 + 响应字节一致 | 部分覆盖（依赖 `env.have_h3()`） | `tests/libcurl_consistency/conftest.py`（nghttpx-h3 注入）+ `tests/libcurl_consistency/test_p0_consistency.py`（h3 变体） | 可选：`QCURL_REQUIRE_HTTP3=1` 强制覆盖（语义来源 `curl/tests/data/test2500`） |
...
| 分类 | 可观测点 | 覆盖结论 | 证据（测试/代码位置） | 缺口任务 |
|---|---|---|---|---|
| 下载 | 文件字节一致（含并发与 Range 续传） | 已覆盖 | `tests/libcurl_consistency/pytest_support/case_defs.py`（download_*）+ `tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（download_*） | - |
...
| 网络路径（M4） | `RESOLVE/CONNECT_TO`：服务端可观测（Host/路径/状态）一致 | 已覆盖 | `tests/libcurl_consistency/test_p1_resolve_connect_to.py` + `tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（caseId：`p1_resolve_override/p1_connect_to`） | - |
| 协议白名单（M5） | `allowedProtocols/allowedRedirectProtocols`：fail-closed + 可观测一致 | 已覆盖 | `tests/libcurl_consistency/test_p2_protocol_restrictions.py` + `tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（caseId：`p2_protocols_block_http/p2_redir_protocols_block_http`） | - |
...
| HTTP 方法面 | HEAD/PATCH/DELETE（无 body）的可观测语义对齐 | 已覆盖（HEAD/PATCH/DELETE） | `tests/libcurl_consistency/test_p1_http_methods.py` + `tests/libcurl_consistency/tst_LibcurlConsistency.cpp` | - |
...
| lc_retry_501_sequence_http_1_1 | （本仓库自建）`tests/libcurl_consistency/test_p2_fixed_http_errors.py::test_p2_retry_501_sequence_http_1_1` | HTTP 501：两次请求序列（retry），序列语义一致 | 已覆盖 | `tests/libcurl_consistency/test_p2_fixed_http_errors.py`（case：`p2_retry_501_sequence_http_1.1`）+ `tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（caseId：`p2_retry_501_sequence`） |
...
| curl_data_test2_basic_auth | `curl/tests/data/test2` | Basic auth（curl CLI `-u`/HTTPAUTH） | 不纳入（不复用 curl CLI 解析语义） | HTTPAUTH 语义已覆盖：`tests/libcurl_consistency/test_p1_httpauth.py`（ANY/ANYSAFE + UNRESTRICTED_AUTH） |
...
- **Cookie 文件读写一致性（`COOKIEFILE/COOKIEJAR`）**
  ...
  - 补充语义来源：`curl/tests/data/test1920`（perform → easy_reset → cleanup 不应丢 cookiejar 内容）；对应一致性用例：`tests/libcurl_consistency/test_p1_cookiejar_1903.py::test_p1_cookiejar_1920`。

- **响应头 unfold（一致性对齐 `curl_easy_header`；折叠行 + TAB）**
  - 语义来源：`curl/tests/data/test1940`（curl_easy_header）以及关联的 `test1274/test798`
  - 说明：验证折叠行与 TAB 的 unfold+trim 行为在 QCurl 的可观测输出中一致；对应一致性用例：`tests/libcurl_consistency/test_p1_resp_headers.py::test_p1_resp_headers_unfold_1940`。
...
- **HTTP proxy（含 HTTPS CONNECT）一致性**
  ...
  - 补充语义来源：`curl/tests/data/test1941`（CONNECT 响应头 folded headers）；默认 Gate 不对 CONNECT 响应头做严格一致性断言，但会做“诊断型采集”（如可观测则落盘到 artifacts；缺失/差异不致失败）。
...
- 一致性测试结构：
  - QCurl 执行器：`tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（通过环境变量选择 case，落盘 `download_*.data`）
  - pytest 驱动与对比器：`tests/libcurl_consistency/pytest_support/*`
  - baseline：
    - 上游 baseline：`build/curl/tests/libtest/libtests`（`LocalClient(name='cli_*')`）
...
- 前置条件（最小集合）：
  - Qt6 + CMake + C++17 编译器
  - `curl/` 子模块已就绪（首次 clone 后执行：`git submodule update --init --recursive`）
  - Python3 + `pytest`（以及 WS 场景的 `websockets`）
  - 可运行的 `curl/tests/http/testenv`（httpd/nghttpx/ws）
- 配置（仅一致性 Gate 需要；会将 curl 构建到 `<qcurl_build>/curl`，默认 `build/curl`）：
  - `cmake -S . -B build -DQCURL_BUILD_LIBCURL_CONSISTENCY=ON`
- 构建 Gate 依赖（唯一入口；含 `tst_LibcurlConsistency` + curl testdeps/libtests + baseline clients）：
  - `cmake --build build --target qcurl_lc_deps -j"$(nproc)"`
- 运行（P0）：
  - `pytest tests/libcurl_consistency/test_p0_consistency.py`
  - （可选覆盖）`QCURL_QTTEST="build/tests/tst_LibcurlConsistency" pytest tests/libcurl_consistency/test_p0_consistency.py`
- 运行（P1）：
  - `python3 tests/libcurl_consistency/run_gate.py --suite p1 --build`
- 运行（all；PR gate：包含 ext；不允许 skipped）：
  - `QCURL_LC_EXT=1 python3 tests/libcurl_consistency/run_gate.py --suite all --with-ext --build`

注意：
- `run_gate.py` 默认“跳过即失败”（见下方 6.7）。`--suite all` 口径下若未启用 `QCURL_LC_EXT=1`，可能出现 `collection skipped` 并触发 gate 失败（`gate_returncode=3`），属于预期行为。
...
说明：
- `tests/libcurl_consistency/conftest.py` 会默认注入 `CURL_BUILD_DIR=build/curl`、`CURL=build/curl/src/curl`、`CURLINFO=build/curl/src/curlinfo`；如需自定义可在环境变量或 `run_gate.py --curl-build` 覆盖。
...
#### 6.4.1 artifacts schema/version 与兼容规则（门禁）

为避免“字段漂移导致误判”，baseline/qcurl 的 `artifacts` JSON 必须声明 schema（格式版本），并由 Gate 校验：

- schema 字段：
  - `schema: "qcurl-lc/artifacts@v1"`（当前唯一允许值）
- v1 兼容规则（默认策略）：
  - ✅ 允许：新增可选字段（Gate 与对比器会忽略未知字段）
  - ❌ 禁止：删除/重命名 required 字段，或改变既有字段语义（视为破坏性变更）
  - 破坏性变更必须 bump major（例如 `@v2`），并同步更新 Gate 与对比器口径
- v1 内的 `error` 字段口径（兼容迁移窗口）：
  - legacy：`error.{kind,http_status,curlcode?,http_code?}`（历史视图，允许仅包含 kind/http_status）
  - canonical：
    - `observed.error.{http_status,http_code?}`：可直接观测字段（来自服务端/协议栈）
    - `derived.error.{kind,curlcode?}`：推导/归一化字段（来自测试侧归一化逻辑）
  - 对比器：优先比较 `observed/derived`，缺失时回退到 legacy `error`（确保旧工件仍可复核）
- Gate 行为：
  - `tests/libcurl_consistency/run_gate.py` 在本次运行结束后做 postflight 校验（仅检查本次生成/更新的 `baseline.json/qcurl.json`）
  - schema 缺失/不匹配会导致 gate 失败（`gate_returncode=3`，`policy_violations` 包含 `artifacts_schema`）
...
- **Gate 默认“跳过即失败”**：为避免“未执行也全绿”，`tests/libcurl_consistency/run_gate.py` 会解析 JUnit 统计；一旦出现 `skipped_tests` 或 `no_tests_executed` 会直接 gate 失败（`gate_returncode=3`，见 `policy_violations`）。
- **HTTP/3 覆盖不等于默认覆盖**：未构建 `qcurl_nghttpx_h3` 或 `env.have_h3()` 为 False 时，h3 变体不会被执行（因此也不会出现在 JUnit 统计中）。如需强制 h3 覆盖，请设置 `QCURL_REQUIRE_HTTP3=1`。
- **受限环境的端口/进程限制**：在 sandbox/容器中可能无法分配端口或启动 httpd/nghttpx/ws，需要相应权限或在宿主环境运行。
- **TLS 观测服务端依赖 CA 生成物**：`lc_observe_https` 复用 `curl/tests/http/gen/ca` 证书产物；首次需要跑一次 curl testenv 以生成 CA/证书（缺失时相关用例会 skip，并触发 gate 失败）。
- **脱敏扫描门禁（零敏感信息泄漏）**：`tests/libcurl_consistency/run_gate.py` 会在本次运行结束后对产物目录执行脱敏扫描（`curl/tests/http/gen/artifacts` + `build/libcurl_consistency/reports`，仅扫描本次生成的文本产物）。若发现 `Authorization/Proxy-Authorization/Cookie/Set-Cookie` 明文则 gate 失败（`gate_returncode=3`）。因此自建观测服务（`http_observe_server.py`/`http_proxy_server.py`）在 JSONL 中仅保留 scheme/摘要，不落盘 header value 明文。
