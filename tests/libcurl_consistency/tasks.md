...
| LC-51 | 补齐 P1：cookie flush/reset 后 cookiejar 落盘一致性（upstream 语义来源：`curl/tests/data/test1920`）。 | 中 | LC-10 | 已完成 | 2026-01-04：观测服务端 `/cookie?scenario=1920` 增加 `Set-Cookie: cookiename=cookiecontent;`；Qt 执行器 `tests/tst_LibcurlConsistency.cpp` 新增 `cookiejar_1920` 分支（`QCURL_LC_COOKIE_PATH` + ReadWrite），pytest 在 `tests/libcurl_consistency/test_p1_cookiejar_1903.py` 新增 `test_p1_cookiejar_1920`（baseline `lib1920` vs QCurl `cookiejar_1920`），并复用 cookiejar 归一化（排序/路径尾随斜杠）。 |
