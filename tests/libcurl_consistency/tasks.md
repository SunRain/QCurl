...
| LC-3 | 在 pytest 中跑 **libcurl baseline**：通过 `LocalClient(name='cli_*')` 调用 `curl/tests/libtest/libtests`，生成 baseline `artifacts`（每条用例一份 JSON）。 | 高 | LC-1、LC-2 | 已完成 | 2025-12-16：完善 `pytest_support/baseline.py`（hash/len、请求语义摘要）；补齐 `case_defs.py` P0 参数模板；新增 `curl/tests/libtest/cli_hx_range_resume.c` 并更新 `curl/tests/libtest/Makefile.inc`，重建 `build/curl/tests/libtest/libtests`。<br>2025-12-16 22:50:45：`cmake --build \"build\" --target qcurl_lc_deps` 构建通过。<br>2025-12-16 22:58:27：为“请求语义摘要”补齐可比字段：上传 baseline 用例参数追加 `-l` 强制 Content-Length；Range 续传 baseline 支持 `-S` 并用 `CURLOPT_RANGE` 显式 end，已重建 `libtests` 并复跑 P0 通过。 |
| LC-4 | 新增 Qt Test（C++）：实现与 P0/P1 对齐的 QCurl 客户端场景（下载/上传/WS），将结果写出为 QCurl `artifacts`（JSON，字段与 LC-0 一致）。 | 高 | LC-0、LC-2 | 已完成 | 2025-12-16：新增 `tests/libcurl_consistency/tst_LibcurlConsistency.cpp`（按 env vars 执行单用例并落盘 download_*.data），并在 `tests/libcurl_consistency/CMakeLists.txt` 注册生成 `build/tests/tst_LibcurlConsistency`；`pytest_support/qcurl_runner.py` 负责运行与写出 QCurl artifacts。<br>2025-12-16 22:50:45：`cmake --build \"build\" --target tst_LibcurlConsistency` 构建通过。 |
...
| LC-51 | 补齐 P1：cookie flush/reset 后 cookiejar 落盘一致性（upstream 语义来源：`curl/tests/data/test1920`）。 | 中 | LC-10 | 已完成 | 2026-01-04：观测服务端 `/cookie?scenario=1920` 增加 `Set-Cookie: cookiename=cookiecontent;`；Qt 执行器 `tests/libcurl_consistency/tst_LibcurlConsistency.cpp` 新增 `cookiejar_1920` 分支（`QCURL_LC_COOKIE_PATH` + ReadWrite），pytest 在 `tests/libcurl_consistency/test_p1_cookiejar_1903.py` 新增 `test_p1_cookiejar_1920`（baseline `lib1920` vs QCurl `cookiejar_1920`），并复用 cookiejar 归一化（排序/路径尾随斜杠）。 |
| LC-52 | 补齐 P1：响应头 unfold（折叠行 + TAB）一致性（upstream 语义来源：`test1940/test1274/test798`），并修复 QCNetworkReply header 解析。 | 中 | LC-26 | 已完成 | 2026-01-04：观测服务端 `/resp_headers?scenario=1940` 手写响应头（Fold/Test/Blank/Set-Cookie 等，含折叠行与 TAB）；修复 `src/QCNetworkReply.cpp`：`parseHeaders()/parseResponseHeaders()` 支持 unfold+trim；Qt 执行器新增 `resp_headers_unfold_1940` 并写出 `headers_unfolded_1940.json`；pytest 在 `tests/libcurl_consistency/test_p1_resp_headers.py` 新增 `test_p1_resp_headers_unfold_1940`（baseline `lib1940` curl_easy_header stdout vs QCurl JSON）。 |
| LC-53 | 补齐 P1：proxy CONNECT folded headers（upstream 语义来源：`curl/tests/data/test1941`）+ CONNECT header blocks 诊断采集（不入门禁）。 | 中 | LC-21 | 已完成 | 2026-01-04：proxy 端 `tests/libcurl_consistency/http_proxy_server.py` 在 CONNECT 响应头注入 folded headers（含 TAB）；新增用例 `proxy_connect_headers_1941`（见 `tests/libcurl_consistency/pytest_support/case_defs.py`）；Qt 执行器在 proxy 路径写出 `response_headers_0.data` 并 best-effort 写 `connect_headers_0.data`；pytest 在 `tests/libcurl_consistency/test_p1_proxy.py` 为 baseline 注入 `--header-out`，并将 CONNECT blocks 写入 `connect_headers_diag`（仅诊断，不升级为严格门禁）。 |
| LC-54 | 增强：HTTP/3 覆盖强制开关（upstream 语义来源：`curl/tests/data/test2500`）。 | 低 | LC-2 | 已完成 | 2026-01-04：`tests/libcurl_consistency/test_p0_consistency.py` 支持 `QCURL_REQUIRE_HTTP3=1`：无 HTTP/3 能力（`env.have_h3()`=False）则 fail；默认仍按 capability-gated 跳过 h3 变体。 |
| LC-55 | 补齐 P2：下载 backpressure 可观测一致性（最小合同：bp_on/bp_off 边沿 + body 字节一致）。 | 中 | LC-15 | 已完成 | 2026-01-15：新增 baseline 可执行 `tests/libcurl_consistency/backpressure_baseline_client.cpp`（target `qcurl_lc_backpressure_baseline`）与 pytest 用例 `tests/libcurl_consistency/test_p2_backpressure_contract.py`；Qt 执行器 `tests/libcurl_consistency/tst_LibcurlConsistency.cpp` 新增 case `p2_backpressure_contract`（落盘 `download_0.data` + `backpressure_events.json`）；对比器 `tests/libcurl_consistency/pytest_support/compare.py` 增加 `backpressure_contract` 字段对比与单侧校验。修复 QCurl：`src/QCNetworkReply.cpp` 将 readAll() 的 backpressure 自动恢复改为 queued 触发，避免重入导致字节段落换序。回归：`python3 tests/libcurl_consistency/run_gate.py --suite p2 --qcurl-build build_lc`（28 passed, 1 skipped；需 escalated）。 |
| LC-56 | 补齐 P2：上传 READFUNC_PAUSE 自动恢复可观测一致性（最小合同：zero-read 非 EOF + 最终响应字节一致）。 | 中 | LC-15、LC-46 | 已完成 | 2026-01-15：新增 baseline 可执行 `tests/libcurl_consistency/upload_pause_resume_baseline_client.cpp`（target `qcurl_lc_upload_pause_resume_baseline`）与 pytest 用例 `tests/libcurl_consistency/test_p2_upload_readfunc_pause_resume.py`；Qt 执行器 `tests/libcurl_consistency/tst_LibcurlConsistency.cpp` 新增 case `p2_upload_readfunc_pause_resume`（落盘 `upload_pause_resume.json`）；对比器 `tests/libcurl_consistency/pytest_support/compare.py` 增加 `upload_pause_resume` 字段对比与单侧校验。回归：`python3 tests/libcurl_consistency/run_gate.py --suite p2 --qcurl-build build_lc`（28 passed, 1 skipped；需 escalated）。 |
...
| LC-44 | ext：速率限制“可取消/不死锁”smoke（`CURLOPT_MAX_RECV_SPEED_LARGE`/`CURLOPT_MAX_SEND_SPEED_LARGE`）。 | 中 | LC-29 | 已完成 | 2026-01-22：baseline `qcurl_lc_http_baseline` 新增 `--max-recv-speed/--max-send-speed`（不支持时明确失败）；Qt 执行器新增 `ext_speed_limit_smoke`（字节阈值触发 cancel）；pytest 新增 `tests/libcurl_consistency/test_ext_speed_limit_smoke.py`，并接入 `run_gate.py --with-ext`。 |
| LC-47 | P1：网络路径覆盖一致性（`CURLOPT_RESOLVE`/`CURLOPT_CONNECT_TO`）。 | 中 | LC-5a | 已完成 | 2026-01-22：baseline `qcurl_lc_http_baseline` 新增 `--resolve/--connect-to`；Qt 执行器新增 `p1_resolve_override/p1_connect_to`；pytest 新增 `tests/libcurl_consistency/test_p1_resolve_connect_to.py`，并接入 `run_gate.py --suite p1/all`。 |
| LC-48 | P2：协议白名单与重定向协议限制一致性（`CURLOPT_PROTOCOLS_STR`/`CURLOPT_REDIR_PROTOCOLS_STR`）。 | 中 | LC-5a | 已完成 | 2026-01-22：baseline `qcurl_lc_http_baseline` 新增 `--allowed-protocols/--allowed-redir-protocols`（不支持时明确失败，禁止 silent skip）；Qt 执行器新增 `p2_protocols_block_http/p2_redir_protocols_block_http`（fail-closed）；pytest 新增 `tests/libcurl_consistency/test_p2_protocol_restrictions.py`，并接入 `run_gate.py --suite p2/all`。 |
...
- 期望可观测输出（弱判据）：
  - 事件存在性/顺序一致：`pause → resume → finished`（各出现一次）
  - 最终下载文件字节一致（hash/len）
- 对比方式：
  - artifacts 可选字段 `pause_resume`：
    - 对比器只比较 `pause_offset/pause_count/resume_count/event_seq`（不比较 pause window 内事件计数）
    - `paused_data_events/paused_progress_events` 仅作为诊断字段输出
- 完成判据：
  - 用例进入 `run_gate.py --suite all`，并可连续运行 10 次无偶发失败
- 最小可复现步骤：
...
- QCurl：`tests/libcurl_consistency/tst_LibcurlConsistency.cpp` 新增 case `p2_socks5_proxy_connect_fail`，通过 `QCNetworkProxyConfig::ProxyType::Socks5` 配置代理，并断言 `reply->error()==fromCurlCode(CURLE_PROXY)`。
...
- QCurl：`tests/libcurl_consistency/tst_LibcurlConsistency.cpp` 中 `p2_expect_100_continue` 改为 `sendPut`，移除手工 `Expect` 头。
