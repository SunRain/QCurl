"""
P1：HTTPAUTH 挑战/协商与跨 host/port 重定向凭据策略一致性。

比较挑战后的 Authorization 行为，以及 `UNRESTRICTED_AUTH` 开关的可观测差异。
"""
...
def _assert_httpauth_challenge_flow(*,
    ...
    # 401 challenge 必须先发生，且首个请求不应携带 Authorization，避免把预发凭据误判为挑战流程。
