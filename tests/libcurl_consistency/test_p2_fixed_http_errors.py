...
def test_p2_retry_501_sequence_http_1_1(env, lc_services, lc_logs, lc_observe_http, tmp_path):
    ...
    proto = "http/1.1"
    status_code = 501
    case_variant = "p2_retry_501_sequence_http_1.1"

    trace_base = f"lc_{uuid.uuid4().hex[:8]}_retry_501_seq"
    baseline_req_id = f"{trace_base}__baseline"
    qcurl_req_id = f"{trace_base}__qcurl"
    ...
    try:
        ...
        observed = observe_http_observed_list_for_id(observe_log, baseline_req_id, expected_count=2)
        baseline["payload"]["requests"] = [
            build_request_semantic(o.method, o.url, o.headers, b"") for o in observed
        ]
        base_resp = baseline["payload"]["response"]
        baseline["payload"]["responses"] = [
            {**dict(base_resp), "status": o.status, "http_version": proto, "headers": o.response_headers}
            for o in observed
        ]
        last = observed[-1]
        baseline["payload"]["request"] = build_request_semantic(last.method, last.url, last.headers, b"")
        ...
        qcurl = run_qt_test(
            ...
            download_count=1,
            case_env={
                "QCURL_LC_CASE_ID": "p2_retry_501_sequence",
                "QCURL_LC_PROTO": proto,
                "QCURL_LC_REQ_ID": qcurl_req_id,
                "QCURL_LC_OBSERVE_HTTP_PORT": str(port),
            },
        ...
        observed = observe_http_observed_list_for_id(observe_log, qcurl_req_id, expected_count=2)
        qcurl["payload"]["requests"] = [
            build_request_semantic(o.method, o.url, o.headers, b"") for o in observed
        ]
        base_resp = qcurl["payload"]["response"]
        qcurl["payload"]["responses"] = [
            {**dict(base_resp), "status": o.status, "http_version": proto, "headers": o.response_headers}
            for o in observed
        ]
        last = observed[-1]
        qcurl["payload"]["request"] = build_request_semantic(last.method, last.url, last.headers, b"")
