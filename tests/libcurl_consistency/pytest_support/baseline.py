...
def _default_share_handle_baseline_binary() -> Path:
    qt_bin = os.environ.get("QCURL_QTTEST", "").strip()
    if qt_bin:
        return Path(qt_bin).resolve().with_name("qcurl_lc_share_handle_baseline")
    return Path("qcurl_lc_share_handle_baseline")
...
def run_libtest_case(
    ...
    elif client_name == "cli_lc_share_handle":
        baseline_executable = (
            Path(os.environ.get("QCURL_LC_SHARE_HANDLE_BASELINE", "")).resolve()
            if os.environ.get("QCURL_LC_SHARE_HANDLE_BASELINE")
            else _default_share_handle_baseline_binary()
        )
        if not baseline_executable.exists():
            raise FileNotFoundError(f"share handle baseline 可执行不存在：{baseline_executable}")
        stdout_lines, stderr_lines, duration_ms, exit_code = _run_standalone_baseline(
            env,
            executable=baseline_executable,
            args=cmd_args,
