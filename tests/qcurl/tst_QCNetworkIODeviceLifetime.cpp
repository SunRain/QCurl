...
public:
    ...
    QUrl url() const
    {
        return QUrl(QStringLiteral("http://127.0.0.1:%1/slow-body").arg(m_server.serverPort()));
    }
...
private:
    void handleSocket(QTcpSocket *socket)
    {
        ...
        QObject::connect(
            socket, &QTcpSocket::readyRead, socket, [this, socket, requestBuffer, requestHandled]() {
                if (*requestHandled) {
                    return;
                }
                ...
                *requestHandled = true;
                ...
                const QByteArray responseHeader = QByteArray(
                                                      "HTTP/1.1 200 OK\r\n"
